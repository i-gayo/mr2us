...
resnet50.fc = nn.Linear(resnet50.fc.in_features, 1) # change final layer to 1 class only 

# Define a custom ResNet50 with a different number of input channels
class ResNet50(nn.Module):
    def __init__(self, num_input_channels=1, num_classes=1):
        ...
        self.conv1.weight.data = resnet50.conv1.weight.data[:, :num_input_channels, :, :]

        # Use the remaining layers from the pre-trained ResNet-50
        self.features = nn.Sequential(*list(resnet50.children())[1:-1])
...
class UNet(nn.Module):
    ...
    def __init__(self, input_channel = 1, output_channel = 1, num_features = 32):
        ...
        self.encoder_1 = UNet._build_conv_block(input_channel, num_features)
        self.maxpool_1 = nn.MaxPool2d(kernel_size = 2, stride = 2)
        self.encoder_2 = UNet._build_conv_block(num_features, num_features*2)
        self.maxpool_2 = nn.MaxPool2d(kernel_size = 2, stride = 2)
        self.encoder_3 = UNet._build_conv_block(num_features*2, num_features*4)
        self.maxpool_3 = nn.MaxPool2d(kernel_size = 2, stride = 2)
        self.encoder_4 = UNet._build_conv_block(num_features*4, num_features*8)
        self.maxpool_4 = nn.MaxPool2d(kernel_size = 2, stride = 2)

        self.bottle_neck = UNet._build_conv_block(num_features *8, num_features * 16)

        self.upconv_4 = nn.ConvTranspose2d(num_features * 16, num_features * 8, kernel_size = 2, stride = 2)
        self.decoder_4 = UNet._build_conv_block((num_features*8)*2, num_features *8)
        self.upconv_3 = nn.ConvTranspose2d(num_features * 8, num_features * 4, kernel_size = 2, stride = 2)
        self.decoder_3 = UNet._build_conv_block((num_features*4)*2, num_features *4)
        self.upconv_2 = nn.ConvTranspose2d(num_features * 4, num_features * 2, kernel_size = 2, stride = 2)
        self.decoder_2 = UNet._build_conv_block((num_features*2)*2, num_features *2)
        self.upconv_1 = nn.ConvTranspose2d(num_features*2 , num_features, kernel_size = 2, stride = 2)
        self.decoder_1 = UNet._build_conv_block(num_features*2, num_features)

        self.final_conv = nn.Conv2d(num_features, output_channel, kernel_size = 1)
    ...
    def _build_conv_block(input_channel, num_features):
        ...
        conv_block = nn.Sequential(
            nn.Conv2d(in_channels = input_channel, out_channels = num_features, kernel_size = 3, padding = 1, bias = False),
